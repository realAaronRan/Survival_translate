...
@FilePath: \Survival_translate\ALE_Switcher.py
@Date: 2020-03-05 22:29:02
@LastEditTime: 2020-03-10 15:12:53
'''

from pandas import read_excel
from xlwings import Book
from multiprocessing import Pool, Manager, cpu_count, freeze_support
from tkinter import ttk, filedialog, Toplevel, Frame, StringVar, BooleanVar, messagebox, INSERT, WORD, END, scrolledtext, Tk
...
class Root(Tk):
    def __init__(self):
        ...
        self.title("ALE Switcher")
        self.height = 500
        self.width = 350
        # self.geometry('250x500')
        self.iconbitmap('translation.ico')
        ...
        # 导入原始文档，并预览
        self.label_raw = ttk.Label(self.file_zone, text="原始文档：")
        ...
        self.male_text = scrolledtext.ScrolledText(self.role_frame, width=48, height=1, wrap=WORD)
        self.male_text.insert(INSERT, '(角色名间请用一个空格分隔，boss名可以写入任意一个框格)')
        ...
        self.female_text = scrolledtext.ScrolledText(self.role_frame, width=48, height=1, wrap=WORD)
    ...
    def exploreRawFile(self):
        self.filename = filedialog.askopenfilename(initialdir="/", title="选择文件", filetype=(("All Files", "*.*"), ("xls", "*.xls"), ("xlsx", "*.xlsx")))
        self.textbox_raw.delete(0, END)
        self.textbox_raw.insert(0, self.filename)
        self.raw_file.set(self.filename)

    def previewRawFile(self):
        try:
            excel_df = read_excel(self.raw_file.get(), sheet_name='Sheet1').head(20)
            ...
            messagebox.showerror(title='Warning', message="无法打开指定文件，请检查是否指定了文件路径，及文件路径是否正确。")

    def previewTransFile(self):
        try:
            excel_df = read_excel(self.trans_file.get(), sheet_name='Sheet1').head(20)
    ...
    def exploreTransFile(self):
        self.filename = filedialog.askopenfilename(initialdir="/", title="选择文件", filetype=(("All Files", "*.*"), ("xls", "*.xls"), ("xlsx", "*.xlsx")))
        self.textbox_trans.delete(0, END)
    ...
    def findAllLanguages(self):
        # 读入excel文件
        try:
            excel_raw = read_excel(self.raw_file.get(), sheet_name='Sheet1', header=None).head()
            ...
            messagebox.showerror(title='Error', message='读入原始文档失败，请检查是否已指定原始文档路径，及路径是否正确。')

    def findMatchedLanguages(self):
        try:
            excel_trans = read_excel(self.trans_file.get(), sheet_name='Sheet1', header=None).head(1)
            ...
            messagebox.showerror(title='Error', message='读入翻译文档失败，请检查是否已指定翻译文档路径，及路径是否正确。')

    def getRoles(self):
        male_roles = self.male_text.get('0.0', END).replace('\n', ' ').split(' ')
        female_roles = self.female_text.get('0.0', END).replace('\n', ' ').split(' ')
        self.roles = {}
    ...
    def startTranslate(self):
        try:
            self.excel_raw = read_excel(self.raw_file.get(), sheet_name='Sheet1')
            self.excel_trans = read_excel(self.trans_file.get(), sheet_name='Sheet1')
            # 检查文档格式是否正确：翻译文档的第一列为基本语言
            first_lang = self.excel_trans.columns.values[0]
            if first_lang != self.base_lang.get():
                raise BaseLangError

            self.column_index_raw = {}
            for index, lang in enumerate(self.excel_raw.columns.values.tolist()):
                self.column_index_raw[lang] = index+1
            self.lang_select_list = [self.valid_lang[i] for i in range(len(self.valid_lang)) if self.lang_select_states[i].get()]

            self.return_dict = Manager().dict()
            cpu_num = int(0.8 * cpu_count())
            pool = Pool(processes=cpu_num)
            for lang in self.lang_select_list:
                sht_raw = self.excel_raw.loc[:, ['类型', self.base_lang.get()]]
                sht_trans = self.excel_trans.loc[:, [self.base_lang.get(), lang]]
